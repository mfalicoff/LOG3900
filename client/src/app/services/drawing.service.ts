...
export class DrawingService {
    ...
    reDrawStand(stand: Tile[], letterBank: Map<string, LetterData>) {
        ...
        for (let x = 0; x < Constants.NUMBER_SLOT_STAND; x++) {
            if (stand[x] !== undefined && stand[x].letter.value !== '') {
                this.drawOneLetter(
                    stand[x].letter.value, stand[x], 
                    this.canvasBoardStand, letterBank, 
                    stand[x].color);
            }
        }
    }
    ...
    resetColorTileStand(player: Player, letterBank: Map<string, LetterData>) {
        for (let x = 0; x < Constants.NUMBER_SLOT_STAND; x++) {
            if (player.stand[x] !== undefined && player.stand[x].letter.value !== '') {
                this.drawOneLetter(
                    player.stand[x].letter.value, 
                    player.stand[x], this.canvasBoardStand, 
                    letterBank, '#F7F7E3');
            }
        }
    }
    ...
    drawOneLetter(
        ...
        canvas.fillRect(
            tileArray.position.x1 + 1, tileArray.position.y1 + 1, 
            tileArray.position.width - 2, tileArray.position.height - 2);
        ...
        this.roundRect(
            tileArray.position.x1 + 1, tileArray.position.y1 + 1, 
            tileArray.position.width - 2, tileArray.position.height - 2, canvas);
        // the number are so the letter tiles are smaller than the tile of the board
        ...
        canvas.fillText(
            letterToDrawUpper, tileArray.position.x1 + spaceForLetter.x, 
            tileArray.position.y1 + spaceForLetter.y);

        canvas.font = '12px bold system-ui';
        if (letterWeight) {
            canvas.fillText(
                letterWeight.toString(), tileArray.position.x1 + spaceForNumber.x, 
                tileArray.position.y1 + spaceForNumber.y);
        } else {
            canvas.fillText(
                '', tileArray.position.x1 + spaceForNumber.x, 
                tileArray.position.y1 + spaceForNumber.y);
        }
    ...
    removeTile(tile: Tile) {
        ...
        this.canvasBoardStand.fillRect(
            tile.position.x1, tile.position.y1, 
            tile.position.width, tile.position.height);
    ...
    drawSpectatorStands(players: Player[]) {
        ...
        this.drawVertiStand(
            paddingForStands + Constants.DEFAULT_WIDTH_BOARD + paddingStandBoard, 
            constPosXYForStands, players[1]);
        this.drawHorizStand(constPosXYForStands, 0, players[2]);
        this.drawHorizStand(
            constPosXYForStands, 
            Constants.DEFAULT_WIDTH_BOARD + paddingForStands + paddingStandBoard,
            players[3]);
    }
    ...
    private drawHorizStand(x: number, y: number, player?: Player){
        ...
        for(let i = x + Constants.SIZE_OUTER_BORDER_STAND, j = 0;
            ...
            this.canvasBoardStand.fillRect(
                i, Constants.SIZE_OUTER_BORDER_STAND + y, 
                Constants.WIDTH_EACH_SQUARE, Constants.WIDTH_EACH_SQUARE);
            ...
            this.roundRect(
                i, y + Constants.SIZE_OUTER_BORDER_STAND,
                Constants.WIDTH_EACH_SQUARE, Constants.WIDTH_EACH_SQUARE, this.canvasBoardStand);
    ...
    private drawVertiStand(x: number, y: number, player?: Player){
        ...
        //If a player has been given, draw the player's stand
        for(let i = y + Constants.SIZE_OUTER_BORDER_STAND, j = 0;
            ...
            this.canvasBoardStand.fillStyle = '#F7F7E3';
            this.canvasBoardStand.fillRect(
                x + Constants.SIZE_OUTER_BORDER_STAND, i, 
                Constants.WIDTH_EACH_SQUARE, Constants.WIDTH_EACH_SQUARE);
